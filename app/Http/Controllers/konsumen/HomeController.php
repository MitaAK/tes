...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $order_bulan_ini = Jual::whereRaw("konsumen_id=? AND status_jual='TIBA'
            ...
            ->addMonths(1)])->count();
        $pengeluaran_bulan_ini = Jual::whereRaw("konsumen_id=? AND status_jual='TIBA'
            AND waktu_pesan>=? AND waktu_pesan<?", [auth()->user()->id, 
            Carbon::today()->firstOfMonth(), Carbon::today()->firstOfMonth()
            ->addMonths(1)])->sum('total_harga');
        $pengeluaran_minggu_ini = Jual::whereRaw("konsumen_id=? AND status_jual='TIBA'
            AND waktu_pesan>=? AND waktu_pesan<?", [auth()->user()->id, 
            Carbon::today()->subDays(6), Carbon::today()->addDays(1)])->sum('total_harga');
        return view('konsumen.home.index', compact('jual', 'rating_5','rating_semua'
        ,'order_minggu_terakhir','order_bulan_ini','pengeluaran_bulan_ini','pengeluaran_minggu_ini'));
